...
(t/deftest get2-fail-bad-url-test
...
(t/deftest get2-fail-bad-token-test
...
(t/deftest post2-bad-token-test
  (let [url api-url
        token "some-dodgy-token"
        content "content"
        content-type "text/plain"
...
(t/deftest post2-bad-request-test
  (let [config (load-config)
        url api-url
        token (:email-api-token config)
        content nil
